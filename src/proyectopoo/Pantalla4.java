                ...
                if (imagePath != null && !imagePath.isEmpty()) {
                    ...
                    if (imageFile.exists()) {
                        try {
                            ...
                            if (img != null) {
                                // Ensure label has valid dimensions before scaling
                                int labelWidth = labelImagenProductoSeleccionado.getWidth();
                                int labelHeight = labelImagenProductoSeleccionado.getHeight();
                                if (labelWidth <= 0 || labelHeight <= 0) {
                                    // Use default dimensions if label not yet sized
                                    labelWidth = 200; 
                                    labelHeight = 200;
                                }
                                ImageIcon icon = new ImageIcon(img.getScaledInstance(labelWidth, labelHeight, Image.SCALE_SMOOTH));
                                labelImagenProductoSeleccionado.setIcon(icon);
                            } else {
                                labelImagenProductoSeleccionado.setIcon(null);
                            }
                        } catch (java.io.IOException e) { // Explicitly qualify IOException

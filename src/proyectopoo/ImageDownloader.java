...
public class ImageDownloader {
    ...
    public static ImageIcon loadImageIcon(String imagePath, int width, int height) {
        ...
        if (!imageFile.exists() || !imageFile.isFile()) {
            System.err.println("Image file not found or not a file: " + imagePath);
            return null;
        }
        try {
            BufferedImage img = ImageIO.read(imageFile);
            if (img != null) {
                if (width <= 0 || height <= 0) { // Use original size if invalid dimensions provided
                    return new ImageIcon(img);
                }
                Image scaledImg = img.getScaledInstance(width, height, Image.SCALE_SMOOTH);
                return new ImageIcon(scaledImg);
            } else {
                System.err.println("Failed to read image (ImageIO.read returned null): " + imagePath);
                return null;
            }
        } catch (IOException e) {
            System.err.println("IOException while reading image " + imagePath + ": " + e.getMessage());
            return null;
        }
    }
}
